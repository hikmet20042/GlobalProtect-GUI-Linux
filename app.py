...
class GlobalProtectApp:
    ...
    def _set_cli_controls_enabled(self, enabled: bool) -> None:
    ...
    def _set_busy(self, busy: bool) -> None:
        self._is_busy = busy
        if busy:
            self._set_cli_controls_enabled(False)
        else:
            self._set_cli_controls_enabled(self._cli_available)

    def _startup_check(self) -> None:
        try:
            self.client.ensure_available()
            self._cli_available = True
            self._set_cli_controls_enabled(True)
            self.refresh_status()
            ...
            self._log(str(exc))
            self._log(
                "Install GlobalProtect CLI first, then restart the app. On Linux Mint, install your company-provided GlobalProtect package."
            )
            messagebox.showwarning("GlobalProtect GUI", str(exc))

    def _run_async(self, action: str, fn, *args) -> None:
        if not self._cli_available:
            self._log("GlobalProtect CLI is unavailable; command skipped.")
            return
        if self._is_busy:
    ...
    def _handle_result(self, action: str, payload: object) -> None:
        ...
        if action == "error":
            ...
            if isinstance(payload, GlobalProtectError) and "not found in PATH" in str(payload):
                self._cli_available = False
                self._set_cli_controls_enabled(False)
    ...
    def refresh_status(self) -> None:
        if not self._cli_available or self._is_busy:
            return
